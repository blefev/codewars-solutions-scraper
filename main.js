...
function generateFilename(index, language, problemName="solution") {
    let extension = extensions[language] || 'txt';
    return index === 0 ? `${problemName}.${extension}` : `${problemName}_${index + 1}.${extension}`;
}
...
async function main() {
    ...
    const solutions = await page.evaluate(() => {
        return [...document.querySelectorAll('.list-item-solutions')].map(solution => {
            ...
            const kyu = solution.querySelector('.item-title span').textContent.replace(/ +/g, '_');
    ...
    for (const solution of solutions) {
        if (!fs.existsSync(path.join(program.output, solution.kyu))) {
            fs.mkdirSync(path.join(program.output, solution.kyu));
	}
	for (let i = 0; i < solution.codeSolutions.length; i++) {
	    fs.writeFileSync(
	        path.join(program.output, solution.kyu, generateFilename(i, solution.languages[i], solution.problemName)),
